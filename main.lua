...
function love.load()
	...
	player.width = 110
	player.height = 110
	player.gunOffset = 30  -- This is half the distance between the two guns
	...
	player.fire = function()
		if player.cooldown <= 0 then
			...
			player.cooldown = 10
			-- Fire from the left gun
			bulletLeft = {}
			bulletLeft.x = player.x + (player.width / 2) - player.gunOffset
			bulletLeft.y = player.y
			table.insert(player.bullets, bulletLeft)
			-- Fire from the right gun
			bulletRight = {}
			bulletRight.x = player.x + (player.width / 2) + player.gunOffset
			bulletRight.y = player.y
			table.insert(player.bullets, bulletRight)
		end
	end
...
function love.draw()
    ...
    -- font size
    font = love.graphics.newFont(14) -- This should be moved to love.load()
